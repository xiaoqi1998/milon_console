...
| sftoken | ValueOf | value_of | 20993 | （只读查询）查询半同质化代币（SFT）的 ValueOf 数据。 |
...
| sftoken | SlotOwnerOf | slot_owner_of | 39122 | （只读查询）查询半同质化代币（SFT）的 SlotOwnerOf 数据。 |
| sftoken | ApprovalOf | approval_of | 63999 | （只读查询）查询半同质化代币（SFT）的 ApprovalOf 数据。 |
...
| social | SocialProfile | social_profile | 5863 | （只读查询）查询社交的 SocialProfile 数据。 |
| social | SourceApp | source_app | 2147 | （只读查询）查询社交的 SourceApp 数据。 |
| social | IsSourceAppPublisher | is_source_app_publisher | 14357 | （只读查询）查询社交的 IsSourceAppPublisher 数据。 |
| social | PublicDataAnchor | public_data_anchor | 63112 | （只读查询）查询社交的 PublicDataAnchor 数据。 |
| social | Community | community | 59279 | （只读查询）查询社交的 Community 数据。 |
| social | IsCommunityAdmin | is_community_admin | 56720 | （只读查询）查询社交的 IsCommunityAdmin 数据。 |
| social | JoinRequest | join_request | 38404 | （只读查询）查询社交的 JoinRequest 数据。 |
| social | CommunityMembership | community_membership | 36616 | （只读查询）查询社交的 CommunityMembership 数据。 |
...
| sftoken | Approve | approve | 34407 | 授权半同质化代币（SFT）（handler: approve）。 |
| sftoken | RevokeApproval | revoke_approval | 57842 | 撤销授权半同质化代币（SFT）（handler: revoke_approval）。 |
| sftoken | Transfer | transfer | 48437 | 转账半同质化代币（SFT）（handler: transfer）。 |
| sftoken | TransferFrom | transfer_from | 19098 | 转账半同质化代币（SFT）（handler: transfer_from）。 |
...
| social | UpsertSocialProfile | upsert_social_profile | 13067 | 调用 社交 的 UpsertSocialProfile 方法（handler: upsert_social_profile）。 |
| social | RegisterSourceApp | register_source_app | 23225 | 注册社交（handler: register_source_app）。 |
| social | AddSourceAppPublisher | add_source_app_publisher | 55820 | 添加社交（handler: add_source_app_publisher）。 |
| social | RemoveSourceAppPublisher | remove_source_app_publisher | 62309 | 移除社交（handler: remove_source_app_publisher）。 |
| social | TransferSourceAppOwner | transfer_source_app_owner | 21283 | 转账社交（handler: transfer_source_app_owner）。 |
| social | DeactivateSourceApp | deactivate_source_app | 52796 | 调用 社交 的 DeactivateSourceApp 方法（handler: deactivate_source_app）。 |
| social | ReactivateSourceApp | reactivate_source_app | 55074 | 调用 社交 的 ReactivateSourceApp 方法（handler: reactivate_source_app）。 |
| social | UpsertPublicDataUri | upsert_public_data_uri | 445 | 调用 社交 的 UpsertPublicDataUri 方法（handler: upsert_public_data_uri）。 |
| social | RetirePublicDataUri | retire_public_data_uri | 17929 | 调用 社交 的 RetirePublicDataUri 方法（handler: retire_public_data_uri）。 |
| social | RestorePublicDataUri | restore_public_data_uri | 25488 | 调用 社交 的 RestorePublicDataUri 方法（handler: restore_public_data_uri）。 |
| social | CreateCommunity | create_community | 18868 | 创建社交（handler: create_community）。 |
| social | UpdateCommunityUri | update_community_uri | 63564 | 更新社交（handler: update_community_uri）。 |
| social | SetCommunityJoinMode | set_community_join_mode | 20649 | 设置社交（handler: set_community_join_mode）。 |
| social | TransferCommunityOwner | transfer_community_owner | 5543 | 转账社交（handler: transfer_community_owner）。 |
| social | DeactivateCommunity | deactivate_community | 32318 | 调用 社交 的 DeactivateCommunity 方法（handler: deactivate_community）。 |
| social | GrantCommunityAdmin | grant_community_admin | 10690 | 调用 社交 的 GrantCommunityAdmin 方法（handler: grant_community_admin）。 |
| social | RevokeCommunityAdmin | revoke_community_admin | 62146 | 撤销授权社交（handler: revoke_community_admin）。 |
| social | JoinCommunity | join_community | 39196 | 调用 社交 的 JoinCommunity 方法（handler: join_community）。 |
| social | RequestJoinCommunity | request_join_community | 57790 | 调用 社交 的 RequestJoinCommunity 方法（handler: request_join_community）。 |
| social | CancelJoinRequest | cancel_join_request | 33199 | 调用 社交 的 CancelJoinRequest 方法（handler: cancel_join_request）。 |
| social | ApproveJoinRequest | approve_join_request | 2182 | 授权社交（handler: approve_join_request）。 |
| social | RejectJoinRequest | reject_join_request | 21924 | 调用 社交 的 RejectJoinRequest 方法（handler: reject_join_request）。 |
| social | LeaveCommunity | leave_community | 7711 | 调用 社交 的 LeaveCommunity 方法（handler: leave_community）。 |
| social | RemoveCommunityMember | remove_community_member | 7071 | 移除社交（handler: remove_community_member）。 |
