...
func TestAccountView_MarshalPostcard(t *testing.T) {
	t.Run("round trip with all fields populated", func(t *testing.T) {
		...
		deserialized, err := postcard.DeserializePostcard(data, func(d *postcard.Deserializer) (api.AccountView, error) {
			var ac api.AccountView
		...
		deserialized, err = postcard.DeserializePostcard(serializer.Bytes(), func(d *postcard.Deserializer) (api.AccountView, error) {
			var ac api.AccountView
			if err = ac.UnmarshalPostcard(d); err != nil {
				return ac, err
			}
...
func TestAccountView_DeserializeErrors(t *testing.T) {
	t.Run("empty data", func(t *testing.T) {
		_, err := postcard.DeserializePostcard([]byte{}, func(d *postcard.Deserializer) (api.AccountView, error) {
			var ac api.AccountView
			if err := ac.UnmarshalPostcard(d); err != nil {
				return ac, err
			}
	...
	t.Run("truncated data - only address bytes", func(t *testing.T) {
		data := make([]byte, 20)
		_, err := postcard.DeserializePostcard(data, func(d *postcard.Deserializer) (api.AccountView, error) {
			var ac api.AccountView
			if err := ac.UnmarshalPostcard(d); err != nil {
				return ac, err
			}
	...
	t.Run("trailing bytes not allowed", func(t *testing.T) {
		...
		_, err = postcard.DeserializePostcard(dataWithTrailing, func(d *postcard.Deserializer) (api.AccountView, error) {
			var ac api.AccountView
			if err = ac.UnmarshalPostcard(d); err != nil {
				return ac, err
			}
