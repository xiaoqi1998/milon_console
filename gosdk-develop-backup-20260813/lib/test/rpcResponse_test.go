...
func TestRpcResponse_MarshalPostcard(t *testing.T) {
	t.Run("round trip with all fields (no error)", func(t *testing.T) {
		original := lib.RpcResponse{
			RequestId: 1234567890,
			Status:    lib.RpcResponseStatusOk,
		...
		deserialized, err := postcard.DeserializePostcard(data, func(d *postcard.Deserializer) (lib.RpcResponse, error) {
			var rsp lib.RpcResponse
	...
	t.Run("round trip with error", func(t *testing.T) {
		...
		original := lib.RpcResponse{
			...
			Status:    lib.RpcResponseStatusNotFound,
			Body:      []byte("not found"),
			Error: &lib.RpcResponseError{
		...
		deserialized, err := postcard.DeserializePostcard(dataBytes, func(d *postcard.Deserializer) (lib.RpcResponse, error) {
			var rsp lib.RpcResponse
			if err = rsp.UnmarshalPostcard(d); err != nil {
				return rsp, err
			}
	...
	t.Run("round trip with error and nil code and data", func(t *testing.T) {
		original := lib.RpcResponse{
			RequestId: 555,
			Status:    lib.RpcResponseStatusInternal,
			Body:      []byte{},
			Error: &lib.RpcResponseError{
...
func TestRpcResponse_DeserializeErrors(t *testing.T) {
	t.Run("empty data", func(t *testing.T) {
		_, err := postcard.DeserializePostcard([]byte{}, func(d *postcard.Deserializer) (lib.RpcResponse, error) {
			var rsp lib.RpcResponse
			if err := rsp.UnmarshalPostcard(d); err != nil {
				return rsp, err
			}
	...
	t.Run("truncated data - only request id", func(t *testing.T) {
		_, err := postcard.DeserializePostcard([]byte{0x01}, func(d *postcard.Deserializer) (lib.RpcResponse, error) {
			var rsp lib.RpcResponse
			if err := rsp.UnmarshalPostcard(d); err != nil {
				return rsp, err
			}
	...
	t.Run("trailing bytes not allowed", func(t *testing.T) {
		...
		_, err = postcard.DeserializePostcard(dataWithTrailing, func(d *postcard.Deserializer) (lib.RpcResponse, error) {
			var rsp lib.RpcResponse
			if err = rsp.UnmarshalPostcard(d); err != nil {
				return rsp, err
			}
