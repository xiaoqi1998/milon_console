...
func TestListResourcePathInfo_UnmarshalListResourcePathListFromRawList(t *testing.T) {
	t.Run("single entry", func(t *testing.T) {
		...
		result, err := api.UnmarshalListResourcePathListFromRawList(rawList)
		assert.NoError(t, err)
		assert.Len(t, result, 1)
		assert.Equal(t, api.RsHash{1, 2, 3}, result[0].RsHash)
	...
	t.Run("multiple entries", func(t *testing.T) {
		...
		assert.Equal(t, api.RsHash{10, 20}, result[0].RsHash)
		assert.Equal(t, "/first", result[0].Path)
		assert.Equal(t, api.RsHash{30, 40, 50}, result[1].RsHash)
		assert.Equal(t, "/second", result[1].Path)
	})

	t.Run("empty list", func(t *testing.T) {
		result, err := api.UnmarshalListResourcePathListFromRawList([][]any{})
		assert.NoError(t, err)
		assert.Len(t, result, 0)
	})

	t.Run("full 18-byte RsHash", func(t *testing.T) {
		rsHashRaw := make([]interface{}, api.RsHashLen)
		for i := 0; i < api.RsHashLen; i++ {
	...
	t.Run("error - item too short", func(t *testing.T) {
		...
		_, err := api.UnmarshalListResourcePathListFromRawList(rawList)
		assert.Error(t, err)
	})
	...
	t.Run("error - RsHash exceeds 18 bytes", func(t *testing.T) {
		rsHashRaw := make([]interface{}, api.RsHashLen+1)
		for i := 0; i <= api.RsHashLen; i++ {
