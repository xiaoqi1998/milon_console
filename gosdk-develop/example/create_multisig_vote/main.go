...
func example(networkConfig milon.Network) {
	...
	//    (MIP-25) binds the whole execute tx (all ix hashes) plus the gated auth subset, so proposal and execute tx are tamper-proof consistent.
	...
	// 3. First vote (vote_init) by participant signerB (signer index 1, weight 2).
	//    Split-payer (no WithPayer): signerB authorizes ix0 + gas (bit63) as a
	//    signer of accountMultiSig (any_signer partial), so gas is charged from
	//    the multisig wallet. weight 2 < threshold 4 -> not ready yet.
	...
	tx, err = lib.NewTransactionBuilder([]api.PackedInstruction{voteInitWire}).
		AddIxAndPayerSig(*accountMultiSig, signerB, 0, lib.MultisigKeySignatureMode{Index: 1, PublicKey: *pkB}). // accountMultiSig: ix0 + gas (bit63), split-payer (member sig)
		Build()
	if err != nil {
	...
	// 4. Second vote by participant signerC (signer index 2, weight 3).
	//    Split-payer again; cumulative weight 2 + 3 >= threshold 4 -> ready.
	...
	tx, err = lib.NewTransactionBuilder([]api.PackedInstruction{voteWire}).
		AddIxAndPayerSig(*accountMultiSig, signerC, 0, lib.MultisigKeySignatureMode{Index: 2, PublicKey: *pkC}). // accountMultiSig: ix0 + gas (bit63), split-payer (member sig)
		Build()
	if err != nil {
	...
	// 5. Query on-chain vote progress: GetVote -> tuple<VoteMeta, sig_bit, ready>.
	...
	// 6. Create a relayer account that sponsors the gas of the execute tx.
	...
	// 7. Execute the transfer with a vote-gated auth-bit-only signature:
	...
	fmt.Printf("accountMultiSig MIL: %d | diff: %d (create multisig gas + voteInit/vote gas + 1000 MIL transfer out) \n", accountMultiSigBalanceFinal, accountMultiSigBalanceClaimFaucet-accountMultiSigBalanceFinal)
